package encrypted.dssb.config.gameprofiles.defaultconfigs;

import encrypted.dssb.config.gameprofiles.GameProfileConfig;
import encrypted.dssb.config.gameprofiles.PossibleItemGroup;
import encrypted.dssb.config.gameprofiles.StartingItem;
import encrypted.dssb.config.gameprofiles.StatusEffect;

import java.util.ArrayList;
import java.util.Arrays;

public class HardProfileConfig extends GameProfileConfig {
    public HardProfileConfig() {
        Name = "Overworld - Hard";
        Effects = new ArrayList<>(Arrays.asList(
            new StatusEffect("slow_falling", 20, 6, true),
            new StatusEffect("regeneration", 20, 100, true)
        ));
        StartingGear = new ArrayList<>(Arrays.asList(
            new StartingItem("minecraft:stone_pickaxe", 1, true, false),
            new StartingItem("minecraft:stone_shovel", 1, true, false),
            new StartingItem("minecraft:stone_axe", 1, true, false),
            new StartingItem("minecraft:stone_sword", 1, true, false),
            new StartingItem("minecraft:bread", 64, true, false)
        ));
        Items = new ArrayList<>(Arrays.asList(
                new PossibleItemGroup(new String[]{"respawn_anchor"}),
                new PossibleItemGroup(new String[]{"beetroot_soup"}),
                new PossibleItemGroup(new String[]{
                        "iron_horse_armor",
                        "golden_horse_armor",
                        "diamond_horse_armor"
                }),
                new PossibleItemGroup(new String[]{
                        "peony",
                        "pink_tulip",
                        "orange_tulip",
                        "oxeye_daisy",
                        "lilac",
                        "lily_of_the_valley",
                        "poppy",
                        "rose_bush",
                        "red_tulip",
                        "sunflower",
                        "white_tulip",
                        "blue_orchid",
                        "cornflower",
                        "azure_bluet",
                        "allium"
                }),
                new PossibleItemGroup(new String[]{
                        "black_wool",
                        "blue_wool",
                        "brown_wool",
                        "cyan_wool",
                        "gray_wool",
                        "green_wool",
                        "lime_wool",
                        "light_blue_wool",
                        "light_gray_wool",
                        "magenta_wool",
                        "orange_wool",
                        "pink_wool",
                        "purple_wool",
                        "red_wool",
                        "white_wool",
                        "yellow_wool"
                }),
                new PossibleItemGroup(new String[]{"enchanting_table"}),
                new PossibleItemGroup(new String[]{"tnt"}),
                new PossibleItemGroup(new String[]{"pumpkin_pie"}),
                new PossibleItemGroup(new String[]{"lodestone"}),
                new PossibleItemGroup(new String[]{"cookie"}),
                new PossibleItemGroup(new String[]{"end_crystal"}),
                new PossibleItemGroup(new String[]{"target"}),
                new PossibleItemGroup(new String[]{"carrot_on_a_stick"}),
                new PossibleItemGroup(new String[]{"enchanted_book"}),
                new PossibleItemGroup(new String[]{
                        "pearlescent_froglight",
                        "verdant_froglight",
                        "ochre_froglight"
                }),
                new PossibleItemGroup(new String[]{"cocoa_beans"}),
                new PossibleItemGroup(new String[]{"minecart"}),
                new PossibleItemGroup(new String[]{"glistering_melon_slice"}),
                new PossibleItemGroup(new String[]{"crossbow"}),
                new PossibleItemGroup(new String[]{
                        "cod",
                        "cooked_cod",
                        "cod_bucket"
                }),
                new PossibleItemGroup(new String[]{"tinted_glass"}),
                new PossibleItemGroup(new String[]{"fern"}),
                new PossibleItemGroup(new String[]{
                        "gold_block",
                        "gold_ingot",
                        "gold_nugget"
                }),
                new PossibleItemGroup(new String[]{"red_sand"}),
                new PossibleItemGroup(new String[]{
                        "honeycomb",
                        "beehive",
                        "honey_bottle"
                }),
                new PossibleItemGroup(new String[]{"experience_bottle"}),
                new PossibleItemGroup(new String[]{"shears"}),
                new PossibleItemGroup(new String[]{"netherrack"}),
                new PossibleItemGroup(new String[]{
                        "piston",
                        "sticky_piston"
                }),
                new PossibleItemGroup(new String[]{"hopper"}),
                new PossibleItemGroup(new String[]{
                        "carrot",
                        "golden_carrot"
                }),
                new PossibleItemGroup(new String[]{"heart_of_the_sea"}),
                new PossibleItemGroup(new String[]{
                        "bucket",
                        "lava_bucket",
                        "water_bucket",
                        "axolotl_bucket",
                        "tadpole_bucket",
                        "milk_bucket",
                        "powder_snow_bucket"
                }),
                new PossibleItemGroup(new String[]{
                        "emerald",
                        "emerald_block"
                }),
                new PossibleItemGroup(new String[]{"hanging_roots"}),
                new PossibleItemGroup(new String[]{"brown_mushroom"}),
                new PossibleItemGroup(new String[]{"sweet_berries"}),
                new PossibleItemGroup(new String[]{"lantern"}),
                new PossibleItemGroup(new String[]{"fermented_spider_eye"}),
                new PossibleItemGroup(new String[]{"cobweb"}),
                new PossibleItemGroup(new String[]{
                        "bamboo",
                        "bamboo_planks",
                        "bamboo_mosaic",
                        "bamboo_block",
                        "stripped_bamboo_block",
                        "bamboo_sign",
                        "bamboo_door",
                        "bamboo_trapdoor",
                        "bamboo_raft",
                        "bamboo_chest_raft"
                }),
                new PossibleItemGroup(new String[]{"chiseled_quartz_block"}),
                new PossibleItemGroup(new String[]{"rooted_dirt"}),
                new PossibleItemGroup(new String[]{"redstone_lamp"}),
                new PossibleItemGroup(new String[]{"big_dripleaf"}),
                new PossibleItemGroup(new String[]{"dead_bush"}),
                new PossibleItemGroup(new String[]{
                        "chiseled_sandstone",
                        "cut_sandstone"
                }),
                new PossibleItemGroup(new String[]{"firework_rocket"}),
                new PossibleItemGroup(new String[]{
                        "black_concrete",
                        "black_concrete_powder",
                        "blue_concrete",
                        "blue_concrete_powder",
                        "brown_concrete",
                        "brown_concrete_powder",
                        "cyan_concrete",
                        "cyan_concrete_powder",
                        "gray_concrete",
                        "gray_concrete_powder",
                        "green_concrete",
                        "green_concrete_powder",
                        "light_blue_concrete",
                        "light_blue_concrete_powder",
                        "light_gray_concrete",
                        "light_gray_concrete_powder",
                        "lime_concrete",
                        "lime_concrete_powder",
                        "magenta_concrete",
                        "magenta_concrete_powder",
                        "orange_concrete",
                        "orange_concrete_powder",
                        "pink_concrete",
                        "pink_concrete_powder",
                        "purple_concrete",
                        "purple_concrete_powder",
                        "red_concrete",
                        "red_concrete_powder",
                        "white_concrete",
                        "white_concrete_powder",
                        "yellow_concrete",
                        "yellow_concrete_powder"
                }),
                new PossibleItemGroup(new String[]{"detector_rail"}),
                new PossibleItemGroup(new String[]{"goat_horn"}),
                new PossibleItemGroup(new String[]{"repeater"}),
                new PossibleItemGroup(new String[]{
                        "white_dye",
                        "yellow_dye",
                        "red_dye",
                        "purple_dye",
                        "pink_dye",
                        "orange_dye",
                        "magenta_dye",
                        "lime_dye",
                        "light_blue_dye",
                        "light_gray_dye",
                        "gray_dye",
                        "green_dye",
                        "cyan_dye",
                        "blue_dye",
                        "brown_dye",
                        "black_dye"
                }),
                new PossibleItemGroup(new String[]{"painting"}),
                new PossibleItemGroup(new String[]{"blast_furnace"}),
                new PossibleItemGroup(new String[]{
                        "exposed_copper",
                        "exposed_cut_copper",
                        "oxidized_copper",
                        "oxidized_cut_copper",
                        "weathered_copper",
                        "weathered_cut_copper"
                }),
                new PossibleItemGroup(new String[]{"lead"}),
                new PossibleItemGroup(new String[]{"cake"}),
                new PossibleItemGroup(new String[]{"name_tag"}),
                new PossibleItemGroup(new String[]{
                        "acacia_hanging_sign",
                        "oak_hanging_sign",
                        "spruce_hanging_sign",
                        "birch_hanging_sign",
                        "jungle_hanging_sign",
                        "dark_oak_hanging_sign",
                        "mangrove_hanging_sign",
                        "bamboo_hanging_sign"
                }),
                new PossibleItemGroup(new String[]{"dropper"}),
                new PossibleItemGroup(new String[]{"trident"}),
                new PossibleItemGroup(new String[]{
                        "melon",
                        "melon_seeds",
                        "melon_slice"
                }),
                new PossibleItemGroup(new String[]{
                        "mud_bricks",
                        "mud",
                        "packed_mud"
                }),
                new PossibleItemGroup(new String[]{
                        "dried_kelp",
                        "dried_kelp_block",
                        "kelp"
                }),
                new PossibleItemGroup(new String[]{"totem_of_undying"}),
                new PossibleItemGroup(new String[]{
                        "iron_ingot",
                        "iron_block",
                        "iron_nugget"
                }),
                new PossibleItemGroup(new String[]{
                        "azalea",
                        "flowering_azalea",
                        "azalea_leaves",
                        "flowering_azalea_leaves"
                }),
                new PossibleItemGroup(new String[]{
                        "copper_block",
                        "cut_copper",
                        "copper_ingot"
                }),
                new PossibleItemGroup(new String[]{"comparator"}),
                new PossibleItemGroup(new String[]{"honey_block"}),
                new PossibleItemGroup(new String[]{"glow_berries"}),
                new PossibleItemGroup(new String[]{"scute"}),
                new PossibleItemGroup(new String[]{"iron_hoe"}),
                new PossibleItemGroup(new String[]{
                        "mossy_cobblestone",
                        "mossy_stone_bricks"
                }),
                new PossibleItemGroup(new String[]{"jukebox"}),
                new PossibleItemGroup(new String[]{
                        "rabbit",
                        "cooked_rabbit",
                        "rabbit_hide",
                        "rabbit_foot",
                        "rabbit_stew"
                }),
                new PossibleItemGroup(new String[]{"chain"}),
                new PossibleItemGroup(new String[]{
                        "golden_axe",
                        "golden_hoe",
                        "golden_pickaxe",
                        "golden_shovel",
                        "golden_sword"
                }),
                new PossibleItemGroup(new String[]{
                        "tropical_fish",
                        "tropical_fish_bucket"
                }),
                new PossibleItemGroup(new String[]{"brewing_stand"}),
                new PossibleItemGroup(new String[]{
                        "pufferfish",
                        "pufferfish_bucket"
                }),
                new PossibleItemGroup(new String[]{"blaze_powder"}),
                new PossibleItemGroup(new String[]{
                        "black_glazed_terracotta",
                        "black_terracotta",
                        "blue_glazed_terracotta",
                        "blue_terracotta",
                        "brown_glazed_terracotta",
                        "brown_terracotta",
                        "cyan_glazed_terracotta",
                        "cyan_terracotta",
                        "gray_glazed_terracotta",
                        "gray_terracotta",
                        "green_glazed_terracotta",
                        "green_terracotta",
                        "light_blue_glazed_terracotta",
                        "light_blue_terracotta",
                        "light_gray_glazed_terracotta",
                        "light_gray_terracotta",
                        "lime_glazed_terracotta",
                        "lime_terracotta",
                        "magenta_glazed_terracotta",
                        "magenta_terracotta",
                        "orange_glazed_terracotta",
                        "orange_terracotta",
                        "pink_glazed_terracotta",
                        "pink_terracotta",
                        "purple_glazed_terracotta",
                        "purple_terracotta",
                        "red_glazed_terracotta",
                        "red_terracotta",
                        "white_glazed_terracotta",
                        "white_terracotta",
                        "yellow_glazed_terracotta",
                        "yellow_terracotta",
                        "terracotta"
                }),
                new PossibleItemGroup(new String[]{"ender_eye"}),
                new PossibleItemGroup(new String[]{"seagrass"}),
                new PossibleItemGroup(new String[]{"egg"}),
                new PossibleItemGroup(new String[]{"mushroom_stew"}),
                new PossibleItemGroup(new String[]{
                        "beetroot",
                        "beetroot_seeds"
                }),
                new PossibleItemGroup(new String[]{
                        "candle",
                        "white_candle",
                        "orange_candle",
                        "magenta_candle",
                        "light_blue_candle",
                        "yellow_candle",
                        "lime_candle",
                        "pink_candle",
                        "gray_candle",
                        "light_gray_candle",
                        "cyan_candle",
                        "purple_candle",
                        "blue_candle",
                        "brown_candle",
                        "green_candle",
                        "red_candle",
                        "black_candle"
                }),
                new PossibleItemGroup(new String[]{"poisonous_potato"}),
                new PossibleItemGroup(new String[]{
                        "tnt_minecart",
                        "hopper_minecart",
                        "chest_minecart",
                        "furnace_minecart"
                }),
                new PossibleItemGroup(new String[]{"iron_door"}),
                new PossibleItemGroup(new String[]{"iron_trapdoor"}),
                new PossibleItemGroup(new String[]{"tripwire_hook"}),
                new PossibleItemGroup(new String[]{"suspicious_stew"}),
                new PossibleItemGroup(new String[]{
                        "cracked_stone_bricks",
                        "chiseled_stone_bricks"
                }),
                new PossibleItemGroup(new String[]{"stonecutter"}),
                new PossibleItemGroup(new String[]{
                        "ancient_debris",
                        "netherite_scrap",
                        "netherite_ingot",
                        "netherite_block",
                        "netherite_helmet",
                        "netherite_boots",
                        "netherite_chestplate",
                        "netherite_leggings",
                        "netherite_sword",
                        "netherite_axe",
                        "netherite_hoe",
                        "netherite_shovel",
                        "netherite_pickaxe"
                }),
                new PossibleItemGroup(new String[]{"ender_pearl"}),
                new PossibleItemGroup(new String[]{"smooth_basalt"}),
                new PossibleItemGroup(new String[]{
                        "diamond",
                        "diamond_block"
                }),
                new PossibleItemGroup(new String[]{"crying_obsidian"}),
                new PossibleItemGroup(new String[]{"bell"}),
                new PossibleItemGroup(new String[]{"diamond_hoe"}),
                new PossibleItemGroup(new String[]{"dispenser"}),
                new PossibleItemGroup(new String[]{"glowstone_dust"}),
                new PossibleItemGroup(new String[]{
                        "snow",
                        "snowball"
                }),
                new PossibleItemGroup(new String[]{
                        "slime_ball",
                        "slime_block"
                }),
                new PossibleItemGroup(new String[]{"fire_charge"}),
                new PossibleItemGroup(new String[]{"lily_pad"}),
                new PossibleItemGroup(new String[]{
                        "raw_gold_block",
                        "raw_gold"
                }),
                new PossibleItemGroup(new String[]{
                        "amethyst_block",
                        "amethyst_shard"
                }),
                new PossibleItemGroup(new String[]{
                        "carved_pumpkin",
                        "jack_o_lantern",
                        "pumpkin",
                        "pumpkin_seeds"
                }),
                new PossibleItemGroup(new String[]{"turtle_helmet"}),
                new PossibleItemGroup(new String[]{"daylight_detector"}),
                new PossibleItemGroup(new String[]{"compass"}),
                new PossibleItemGroup(new String[]{"writable_book"}),
                new PossibleItemGroup(new String[]{"activator_rail"}),
                new PossibleItemGroup(new String[]{"fishing_rod"}),
                new PossibleItemGroup(new String[]{"cauldron"}),
                new PossibleItemGroup(new String[]{"cactus"}),
                new PossibleItemGroup(new String[]{
                        "raw_copper_block",
                        "raw_copper"
                }),
                new PossibleItemGroup(new String[]{
                        "echo_shard",
                        "recovery_compass"
                }),
                new PossibleItemGroup(new String[]{"phantom_membrane"}),
                new PossibleItemGroup(new String[]{"golden_apple"}),
                new PossibleItemGroup(new String[]{"slime_block"}),
                new PossibleItemGroup(new String[]{"obsidian"}),
                new PossibleItemGroup(new String[]{"flint_and_steel"}),
                new PossibleItemGroup(new String[]{"redstone_torch"}),
                new PossibleItemGroup(new String[]{"iron_bars"}),
                new PossibleItemGroup(new String[]{"spyglass"}),
                new PossibleItemGroup(new String[]{
                        "potato",
                        "baked_potato"
                }),
                new PossibleItemGroup(new String[]{
                        "lapis_block",
                        "lapis_lazuli"
                }),
                new PossibleItemGroup(new String[]{"magma_block"}),
                new PossibleItemGroup(new String[]{"sea_pickle"}),
                new PossibleItemGroup(new String[]{"observer"}),
                new PossibleItemGroup(new String[]{
                        "salmon",
                        "cooked_salmon",
                        "salmon_bucket"
                }),
                new PossibleItemGroup(new String[]{"anvil"}),
                new PossibleItemGroup(new String[]{
                        "acacia_boat",
                        "acacia_chest_boat",
                        "acacia_door",
                        "acacia_log",
                        "acacia_planks",
                        "stripped_acacia_log",
                        "acacia_sapling",
                        "acacia_sign",
                        "acacia_trapdoor"
                }),
                new PossibleItemGroup(new String[]{"hay_block"}),
                new PossibleItemGroup(new String[]{"scaffolding"}),
                new PossibleItemGroup(new String[]{
                        "iron_boots",
                        "iron_chestplate",
                        "iron_helmet",
                        "iron_leggings"
                }),
                new PossibleItemGroup(new String[]{
                        "raw_iron_block",
                        "raw_iron"
                }),
                new PossibleItemGroup(new String[]{"clock"}),
                new PossibleItemGroup(new String[]{"powered_rail"}),
                new PossibleItemGroup(new String[]{"smithing_table"}),
                new PossibleItemGroup(new String[]{
                        "red_sandstone",
                        "cut_red_sandstone",
                        "chiseled_red_sandstone"
                }),
                new PossibleItemGroup(new String[]{
                        "dark_prismarine",
                        "prismarine",
                        "prismarine_bricks",
                        "prismarine_crystals",
                        "prismarine_shard",
                        "sea_lantern"
                }),
                new PossibleItemGroup(new String[]{"note_block"}),
                new PossibleItemGroup(new String[]{"nautilus_shell"}),
                new PossibleItemGroup(new String[]{"red_mushroom"}),
                new PossibleItemGroup(new String[]{
                        "jungle_boat",
                        "jungle_chest_boat",
                        "jungle_door",
                        "jungle_log",
                        "jungle_planks",
                        "jungle_sapling",
                        "jungle_sign",
                        "jungle_trapdoor",
                        "stripped_jungle_log"
                }),
                new PossibleItemGroup(new String[]{"grindstone"}),
                new PossibleItemGroup(new String[]{"golden_chestplate",
                        "golden_helmet",
                        "golden_leggings",
                        "golden_boots"
                }),
                new PossibleItemGroup(new String[]{
                        "black_stained_glass",
                        "blue_stained_glass",
                        "brown_stained_glass",
                        "cyan_stained_glass",
                        "gray_stained_glass",
                        "green_stained_glass",
                        "light_blue_stained_glass",
                        "light_gray_stained_glass",
                        "lime_stained_glass",
                        "magenta_stained_glass",
                        "orange_stained_glass",
                        "pink_stained_glass",
                        "purple_stained_glass",
                        "red_stained_glass",
                        "white_stained_glass",
                        "yellow_stained_glass"
                }),
                new PossibleItemGroup(new String[]{"saddle"}),
                new PossibleItemGroup(new String[]{
                        "sponge",
                        "wet_sponge"
                }),
                new PossibleItemGroup(new String[]{"lightning_rod"}),
                new PossibleItemGroup(new String[]{"rail"}),
                new PossibleItemGroup(new String[]{"arrow"}),
                new PossibleItemGroup(new String[]{
                        "mangrove_boat",
                        "mangrove_chest_boat",
                        "mangrove_door",
                        "mangrove_log",
                        "mangrove_planks",
                        "mangrove_propagule",
                        "mangrove_sign",
                        "mangrove_trapdoor",
                        "stripped_mangrove_log"
                }),
                new PossibleItemGroup(new String[]{
                        "redstone",
                        "redstone_block"}),
                new PossibleItemGroup(new String[]{
                        "diamond_boots",
                        "diamond_chestplate",
                        "diamond_leggings",
                        "diamond_helmet"
                })
        ));

        Items = getItemsWithAvailableAsset(Items);
    }
}
