...
public class BingoHelpCommand {
    ...
    private static int execute(ServerPlayerEntity player) {
        ...
        MessageHelper.sendSystemMessage(player, Text.literal("/bingo preset <preset>: ").formatted(Formatting.GREEN).append(Text.literal("Set the game settings to a predefined preset.").formatted(Formatting.WHITE)));
        ...
        MessageHelper.sendSystemMessage(player, Text.literal("/bingo settings equipment <Add|Remove|Clear|Stone|Iron|Diamond|Food>: ").formatted(Formatting.GREEN).append(Text.literal("Set the type of starting equipment to begin the game with.").formatted(Formatting.WHITE)));
